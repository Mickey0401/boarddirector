...
class Account(models.Model):
    ...
    send_notification = models.BooleanField(_('send email notifications'), default=True)
    send_invitations = models.BooleanField(_('send automatic email invitations for new users and meetings'), default=False)
    ...
    enforce_2fa = models.BooleanField(_('enforce 2fa'), default=False)
    enable_social_logins = models.BooleanField(_('enable social logins'), default=True)
    ...
    def trial_till_date(self):
        ...
        if self.unlimited_trial:
            return timezone.now() + timezone.timedelta(days=363)
    ...
    def _create_subscription(self, elements_token=None):
        ...
        try:
            ...
            errs = e.json_body.get('error', {}) if e.json_body is not None else {}

            errtxt = "\n".join(["%s = %s" % (k, v) for k, v in errs.items()])
            mail_admins('Stripe Error', "@_create_subscription\n%s\n%s" % (errtxt, str(e)), fail_silently=True)
            logger.error("stripe error %s", str(e))

            data = {'status': 'error', 'msg': errs.get('message', 'Something went wrong..')}
    ...
    def _update_card(self, token=None):
        ...
        try:
            ...
            errtxt = "\n".join(["%s = %s" % (k, v) for k, v in errs.items()])
            mail_admins('Stripe Error', "@_update_card\n%s\n%s" % (errtxt, str(e)), fail_silently=True)
            logger.error("stripe error %s", str(e))

            return {'status': 'error', 'msg': errs.get('message', 'Something went wrong...')}
    ...
    def get_path_to_discussion(self):
        return reverse('discussion:organization', kwargs={'url': self.url})
