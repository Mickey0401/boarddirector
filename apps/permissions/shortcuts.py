    ...
    if membership.role == membership.ROLES.assistant and level == 1:
        ...
        # Documents
            ...
            if obj.folder is not None and obj.folder.can_add_files:
                # at first we are checking permissions for folder as for object
                # and then it might be checked as hierarhy of folders
                permissions = permissions.union(_get_object_permissions(membership, obj.folder))
...
def has_object_permission(membership, obj, permission):
    rc = obj is not None and permission in get_object_permissions(membership, obj)
    return rc
